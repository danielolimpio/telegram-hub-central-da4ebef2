...
import { Users, TrendingUp, Star, Clock, SmilePlus } from "lucide-react";
...
const GruposHumor = () => {
    ...
    const fetchGroups = async () => {
      try {
        const { data, error } = await supabase
          ...
          .select("*")
          .eq("status", "approved")
          .eq("category", "Grupos do Telegram de Humor")
  ...
  const seo = categorySEO["humor"];

  return (
    <div className="min-h-screen bg-gradient-subtle">
      <SEOHead title={seo.title} description={seo.description} canonical={seo.canonical} />
      <CategorySchema name="Grupos do Telegram de Humor" description={seo.description} url={`https://gruposdotelegram.org${seo.canonical}/`} />
      <BreadcrumbSchema items={[{ name: "Início", url: "https://gruposdotelegram.org/" }, { name: "Humor", url: `https://gruposdotelegram.org${seo.canonical}/` }]} />
      <Header />
              ...
              <div className="flex items-center mb-4">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground mt-1">
                    Piadas, vídeos engraçados e o melhor do humor brasileiro
                  </p>
                </div>
              </div>
            </div>
