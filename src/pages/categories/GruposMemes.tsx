...
import { Users, TrendingUp, Star, Clock, Camera } from "lucide-react";
...
const GruposMemes = () => {
    ...
    const fetchGroups = async () => {
      try {
        const { data, error } = await supabase
          ...
          .select("*")
          .eq("status", "approved")
          .eq("category", "Grupos do Telegram de Memes")
  ...
  const seo = categorySEO["memes"];

  return (
    <div className="min-h-screen bg-gradient-subtle">
      <SEOHead title={seo.title} description={seo.description} canonical={seo.canonical} />
      <CategorySchema name="Grupos do Telegram de Memes" description={seo.description} url={`https://gruposdotelegram.org${seo.canonical}/`} />
      <BreadcrumbSchema items={[{ name: "Início", url: "https://gruposdotelegram.org/" }, { name: "Memes", url: `https://gruposdotelegram.org${seo.canonical}/` }]} />
      <Header />
              ...
              <div className="flex items-center mb-4">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground mt-1">
                    Os melhores memes da internet brasileira atualizados todo dia
                  </p>
                </div>
              </div>
            </div>
