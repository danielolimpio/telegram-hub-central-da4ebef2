...
import { Users, TrendingUp, Star, Clock, Coins } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import type { Tables } from "@/integrations/supabase/types";

type Group = Tables<"groups">;

const GruposFinancas = () => {
    ...
    const fetchGroups = async () => {
      try {
        const { data, error } = await supabase
          ...
          .select("*")
          .eq("status", "approved")
          .eq("category", "Grupos do Telegram de Finanças Pessoais")
  ...
  const seo = categorySEO["financas"];

  return (
    <div className="min-h-screen bg-gradient-subtle">
      <SEOHead title={seo.title} description={seo.description} canonical={seo.canonical} />
      <CategorySchema name="Grupos do Telegram de Finanças Pessoais" description={seo.description} url={`https://gruposdotelegram.org${seo.canonical}/`} />
      <BreadcrumbSchema items={[{ name: "Início", url: "https://gruposdotelegram.org/" }, { name: "Finanças", url: `https://gruposdotelegram.org${seo.canonical}/` }]} />
      <Header />
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col lg:flex-row gap-4 lg:gap-8">
          <div className="flex-1">
              ...
              <div className="flex items-center mb-4">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground mt-1">
                    Educação financeira, controle de gastos e renda extra
                  </p>
                </div>
              </div>
            </div>
            ...
            <CategoryFAQ categoryKey="financas" />
          </div>
